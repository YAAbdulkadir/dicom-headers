...
async function scanDirectory(jobId, rootPath, _options) {
    var _a, _b, _c;
    ...
    const modalityBySeries = {}; // count series by modality
    let processed = 0;
    for (const file of files) {
        processed++;
        if (processed % 50 === 0 || processed === total) {
            node_worker_threads_1.parentPort?.postMessage({ type: 'progress', jobId, percent: processed / total, currentPath: file });
        }
        let buf;
        try {
        ...
        patients[patientID] || (patients[patientID] = { name: patientName, studies: {} });
        (_a = patients[patientID]).name ?? (_a.name = patientName);
        const study = (_b = patients[patientID].studies)[studyUID] || (_b[studyUID] = {
            studyUID,
            studyDescription: nat.StudyDescription || null,
            series: {}
        });
        const date = nat.AcquisitionDate ||
            nat.InstanceCreationDate ||
            nat.ContentDate ||
            nat.SeriesDate || null;
        const time = nat.AcquisitionTime ||
        ...
        const arr = ((_c = study.series)[seriesUID] || (_c[seriesUID] = []));
        arr.push(inst);
        instancesCount++;
    }
    // final progress 100%
    node_worker_threads_1.parentPort?.postMessage({ type: 'progress', jobId, percent: 1, currentPath: null });
    ...
    node_worker_threads_1.parentPort?.postMessage({ type: 'result', jobId, index: { patients: outPatients, stats } });
}
let FULL_DICT = {};
const nodeRequire = (() => {
    try {
        ...
        try {
            return (0, node_module_1.createRequire)(path.join(process.cwd(), 'package.json'));
        }
        catch {
            return (0, node_module_1.createRequire)(path.join(process.cwd(), 'index.js'));
        }
    }
})();
...
function loadFullDict() {
    ...
    try {
        const mod = nodeRequire('dicom-data-dictionary');
...
function lookupDict(tagHex) {
    if (!FULL_DICT || !Object.keys(FULL_DICT).length)
        FULL_DICT = loadFullDict();
    const variants = keyVariants(tagHex);
    for (const k of variants) {
        const hit = FULL_DICT[k];
        if (hit && typeof hit === 'object') {
            const keyword = (hit.keyword || hit.name || '').toString();
            const vr = (hit.vr || null);
            return { keyword, vr };
        }
    }
    // Not found
...
function readNumericValues(ds, tag, el, vr) {
    ...
    switch (vr) {
        case 'US':
            for (let i = 0; i < n16; i++)
                out.push(ds.uint16(tag, i));
            break;
        case 'SS':
            for (let i = 0; i < n16; i++)
                out.push(ds.int16(tag, i));
            break;
        case 'UL':
            for (let i = 0; i < n32; i++)
                out.push(ds.uint32(tag, i));
            break;
        case 'SL':
            for (let i = 0; i < n32; i++)
                out.push(ds.int32(tag, i));
            break;
        case 'FL':
            for (let i = 0; i < n32; i++)
                out.push(ds.float(tag, i));
            break;
        case 'FD':
            for (let i = 0; i < n64; i++)
                out.push(ds.double(tag, i));
            break;
        case 'AT':
...
;
(async () => {
    const { jobId, rootPath, options, mode, singleFile } = node_worker_threads_1.workerData || {};
    try {
        if (mode === 'headers' && singleFile) {
            const headers = headersForFile(singleFile);
            node_worker_threads_1.parentPort?.postMessage({ type: 'headers', jobId, headers });
            return;
        }
        await scanDirectory(jobId, rootPath, options);
    }
    catch (err) {
        node_worker_threads_1.parentPort?.postMessage({ type: 'error', jobId, error: String(err?.message || err) });
    }
})();
